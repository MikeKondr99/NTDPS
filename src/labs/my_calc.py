...
_priorities:dict[str,int] = {'+':0 , '-':0, '/':1, '*':1, '^':2,'(': -1,'':-1,'u+':5, 'u-':5}
_unary:set[str] = {'+','-'}
...
def rpn(expr: str) -> list[str]:
    ...
    for token in tokens:
        if token == '':
            ...
            if not operand: raise Exception(f"Ожидался оператор, вместо '{token}'")
            ops.append(token)
            plevel += 1
            operand = True
            ...
            if _priorities[token] > _priorities[ops[-1]] or (token == '^' and ops[-1] == '^'):
                ops.append(token)
            else:
            ...
            if operand: raise Exception(f"Ожидался операнд, вместо '{token}'")
