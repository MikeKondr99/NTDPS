...
class GamesData:
    ...
    def _add_win(self, team: str) -> None:
        self.data[team] = cast(
            ...
            tuple(map(lambda x, y: x + y, self.data[team], (1, 1, 0, 0, 3))),
        )

    def _add_lose(self, team: str) -> None:
        self.data[team] = cast(
            Tuple[int, int, int, int, int],
            tuple(map(lambda x, y: x + y, self.data[team], (1, 0, 0, 1, 0))),
        )

    def add_tie(self, team: str) -> None:
        self.data[team] = cast(
            Tuple[int, int, int, int, int],
            tuple(map(lambda x, y: x + y, self.data[team], (1, 0, 1, 0, 1))),
        )
